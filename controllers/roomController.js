...
export const createRoom = async (req, res, next)=>{
    ...
    try{
        ...
        try{
            await Hotel.findByIdAndUpdate(hotelId, {$push: {rooms: savedRoom._id}}) //updating room id from Room model to the Hotel model
        } catch(err){
...
export const updateRoom = async (req, res, next)=>{
    try{
        const updateRoom = await Room.findByIdAndUpdate(req.params.id, {$set: req.body}, {new: true}) //updating data in the database, {new: true} is used so that it return the updated record
...
export const deleteRoom = async (req, res, next)=>{
    ...
    try{
        ...
        try{
            await Hotel.findByIdAndUpdate(hotelId, {$pull: {rooms: req.params.id}}) //deleting room of a hotel, the id received from params must be an existing room id of a hotel
        } catch(err){
            next(err)//execute the middleware to handle error in index.js file
        }
