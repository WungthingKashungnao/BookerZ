...
export const login = async (req, res, next) => {
  try {
    ...
    if (!isPasswordCorrect)
      return next(createError(400, "Wrong password or username!")); //calling custom function to handle error

    //creating jwt token if password is correct
    const token = jwt.sign(
      { id: user._id, isAdmin: user.isAdmin },
      process.env.JWT
    ); //we are going to send this token for each request to verify our identity
    ...
    res.status(200).json({
      ...
      theUser: { details: { ...otherDetails }, isAdmin },
    });
  } catch (err) {
    next(err);
  }
